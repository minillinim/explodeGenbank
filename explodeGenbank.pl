...
#    explodeGenbank.pl
#    
#    Separate a multiple genbank file into separate records
#
...
# CODE HERE
######################################################################
my $gb_fh = openRead($global_options->{'genbank'});
my $in_gb = 0;
my $gb_file_open = 0;
my @gb_buffer = ();
my $gbr_fh;
my $ex_dir = $global_options->{'genbank'}."_exploded"; 
make_path($ex_dir);
while(<$gb_fh>)
{
    if($_ =~ /^LOCUS/)
    {
        # new GB record
        $in_gb = 1; 
        # no file opened yet, push onto the buffer
        push @gb_buffer, $_;
    }
    elsif($_ =~ /^\/\//)
    {
    ...
    elsif($_ =~ /^VERSION/)
    {
        ...
        $line =~ s/ .*//;
        $line .= ".gb";
        $gbr_fh = openWrite($ex_dir."/".$line);
        $gb_file_open = 1;
        foreach my $b_line (@gb_buffer)
        {
            print $gbr_fh $b_line;
        }
        print $gbr_fh $_;
...
sub checkParams {
    ...
    #
    my @standard_options = ( "help|h+", "genbank|g:s");
    my %options;
...
=head1 DESCRIPTION
   ...
   Split multiple genbank records into separate files

=head1 SYNOPSIS

    explodeGenbank.pl  -genbank|g GB_FILE

      -genbank -g GB_FILE          Genbank file to explode
